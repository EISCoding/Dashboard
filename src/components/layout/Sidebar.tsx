...
export function Sidebar({ active, collapsed, onChange, onToggle }: Props) {
  return (
    ...
    >
      ...
      <nav
        className={[
          "flex-1 space-y-2 px-3 py-5",
          collapsed ? "flex flex-col items-center" : "flex flex-col",
        ].join(" ")}
      >
        {items.map((item) => {
          ...
          return (
            <button
              ...
              className={[
                "group flex w-full items-center gap-3 rounded-2xl border text-left transition",
                collapsed
                  ? "h-14 w-14 justify-center self-center px-0 py-0"
